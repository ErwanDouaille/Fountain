...
#define LG_KINECTGENERATORV2_OUTPUT_NONE 0x00
//! Value for defining RGB image as parameter
#define LG_KINECTGENERATORV2_OUTPUT_RGB 0x01
//! Value for defining IR image as parameter
#define LG_KINECTGENERATORV2_OUTPUT_IR 0x02
//! Value for defining DEPTH image as parameter
#define LG_KINECTGENERATORV2_OUTPUT_DEPTH 0x04
//! Value for defining users map as parameter
#define LG_KINECTGENERATORV2_OUTPUT_USERS 0x08
//! Value for defining any device as input
#define LG_KINECTGENERATORV2_INPUT_ANY "LG_KINECTGENERATORV2_INPUT_ANY"

class MicrosoftKinectV2Generator: public Generator
{
private:
	...
	*/
	...
	/*!
	* \brief Rotate and translate the point according to stored data
	* \param[in] CameraSpacePoint : CameraSpacePoint in Kinect camera coordinates
	* \return a Point3D in global coordinates
	*/
	Point3D getRepositionedPoint(CameraSpacePoint);

public:
	MicrosoftKinectV2Generator(string name);
	~MicrosoftKinectV2Generator(void);

	void RegisterBody(INT64 nTime, int nBodyCount, IBody** ppBodies, map<string,Group3D*>& g3D,map<string,Group2D*>& g2D);

	bool initKinect(void);

	/*!
	* \brief Get a pointer to a copy of this MicrosoftKinectGenerator
	...
	*/
	...
	/*!
	* \brief Start the MicrosoftKinectGenerator by initializing streams
	...
	*/
	...
	/*!
	* \brief Stop the MicrosoftKinectGenerator by closing streams
	* \return true if success
	*/
	bool stop();
	...
	*/
	...
	/*!
	* \brief Enable/disable the use of the camera angle
	* \param[in] bool : use camera angle
	*/
	void useCameraElevation(bool) ;
	...
	*/
	...
	/*!
	* \brief Get the color frame reader
	body frame reader
	* \return The color frame reader
	*/
	IColorFrameReader* getColorFrameReader() {return pColorReader;}

	/*!
	* \brief Get the body frame reader
	* \return The body frame reader
	*/
